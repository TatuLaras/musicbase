...
fn get_by() {
    ...
    let artists = db.get_all::<Artist>(Order::Default).unwrap();
    let albums = db.get_all::<Album>(Order::Default).unwrap();
    let songs = db.get_all::<Song>(Order::Default).unwrap();
    ...
    assert_eq!(
        db.get_by::<Artist>(gt("artist_id", "1"), Order::Default)
            .unwrap()[0]
            .name,
